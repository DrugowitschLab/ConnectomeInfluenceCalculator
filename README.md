...
(equivalent HTTP: <https://storage.googleapis.com/lee-lab_brain-and-nerve-cord-fly-connectome/compiled_data/banc_888/banc_888_edgelist_simple_v2.feather>). Drop it into `InfluenceCalculator.from_feather('banc_888_edgelist_simple_v2.feather', meta_df=meta_df)` once downloaded.
...
The same calculator can be built from an in-memory pandas DataFrame or several common on-disk formats — pick whichever matches your pipeline:

```python
ic = InfluenceCalculator.from_dataframes(edges_df, meta_df)
...
Raw influence scores from `calculate_influence` span many orders of magnitude — the leading eigenmode of `(I - W̃)⁻¹` is amplified by `1 / (1 - lambda_max)` (≈ 100× at the default `lambda_max=0.99`), and weakly connected nodes pick up vanishingly small values that crowd the lower tail. `adjust_influence` makes the output legible by taking the natural log and shifting it so the smallest meaningful score sits at zero:
...
ic = InfluenceCalculator.from_dataframes(
...
adjusted = adjust_influence(ic.calculate_influence(seeds.tolist()),
                            const=15)
```
...
If you use the influence scores produced by this library in published work, please cite Bates, Phelps, Kim, Yang et al. (2025), where the algorithm is applied across the BANC whole-CNS *Drosophila* connectome:

> Bates AS, Phelps JS, Kim M, Yang HH, *et al.* **Distributed control circuits across a brain-and-cord connectome.** *bioRxiv* (2025). doi:[10.1101/2025.07.31.667571](https://doi.org/10.1101/2025.07.31.667571). PMID:[40766407](https://pubmed.ncbi.nlm.nih.gov/40766407/).
